...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("\n¡Binevenido al sistema Conversor de Monedas ONE-G8!\n");
        
        do {
            System.out.println("\n------------------------------------------------------------" +
                    "\n\n¿Desde qué tipo de moneda vas a convertir?");

            for (int i = 0; i < seed.currencies.size(); i++) {
                System.out.println("\t" + (i+1) + ") " + seed.currencies.get(i).code());
            }
            ...
            System.out.print("\nIngrese el monto (en " + seed.currencies.get(option1 - 1).code() + ") a convertir: $");
            double amount = scanner.nextDouble();
            scanner.nextLine();

            try {
                Conversion result = convert.convertAmountFromTo(amount, seed.currencies.get(option1-1).code(), seed.currencies.get(option2-1).code());
                if (result.result().equalsIgnoreCase("success")) {
                    System.out.println("\n\nRESULTADO: $" + amount + " " + seed.currencies.get(option1-1).code() +
                            " equivalen a $" + result.conversion_result() + " " + seed.currencies.get(option2-1).code() + ".");
                } else
